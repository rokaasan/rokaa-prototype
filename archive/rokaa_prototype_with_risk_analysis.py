...
if st.session_state.get("ready_for_assessment"):
    ...
    if st.button("Submit Assessment"):
        ...
        if "Europe" in markets or "North America" in markets:
            if dg_score < 60:
                risks.append("⚠️ *Regulatory Exposure*: Operating in international markets (e.g. GDPR, CCPA) without strong governance policies.")

        if dq_score < 50:
            risks.append("⚠️ *Poor Data Quality*: Decisions may be impacted by inaccurate, inconsistent or incomplete data.")

        if mm_score < 50:
            risks.append("⚠️ *Metadata Gaps*: Lack of metadata management can slow down access to trusted data and increase redundancy.")

        if "ai" in goals.lower() or "machine learning" in goals.lower():
            if dq_score < 60 or dg_score < 60:
                risks.append("⚠️ *AI Risk*: AI/ML ambitions require strong data governance and quality — current levels may pose risks to model performance.")

        if segment == "Startup" and dg_score < 40:
            risks.append("⚠️ *Foundational Risk*: Startups often overlook governance early — building a culture of responsible data use is critical.")
